...
2. Load and validate the user's regulation pack using [regulation-pack-schema.md](references/regulation-pack-schema.md). Prefer local sources; only use configured official domains for online retrieval. Record title, issuer, version/effective date, locator, URL/path, and retrieval date. Build a local index with `scripts/build_index.py` and retrieve candidates with `scripts/retrieve.py`; do not place the entire regulation corpus in context.
3. Extract paragraphs and tables with `scripts/extract_docx.py`. Preserve paragraph/table indexes and text hashes for evidence locations. Retrieve relevant rules per paragraph/table and include only the top candidates plus their source metadata in the review context.
